class CoursesController < ApplicationController
  ...
  def load_courses
    @courses = Course.includes(:department, department: :school).all
  end

  def filter_courses
    params[:filter].try(:permit!)
  ...
  def filter_by_credits
    @courses = @courses.where(credits: params[:filter][:credits])
  end
  ...
  def clean_params
    if params.dig(:filter, :credits).present?
      params[:filter][:credits] = params[:filter][:credits].keys
    end
  end
end
